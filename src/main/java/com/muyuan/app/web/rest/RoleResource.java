...
/**
 * REST controller for managing {@link com.muyuan.app.domain.Role}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Role> partialUpdateRole(
        ...
        Optional<Role> result = roleRepository
            ...
            .map(existingRole -> {
                ...
                if (role.getOrderBy() != null) {
                    existingRole.setOrderBy(role.getOrderBy());
                }
                if (role.getNote() != null) {
                    existingRole.setNote(role.getNote());
                }
